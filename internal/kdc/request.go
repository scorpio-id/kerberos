...
import (
	...
	"github.com/scorpio-id/kerberos/internal/iana/msgtype"
	"github.com/scorpio-id/kerberos/internal/iana/nametype"
	"github.com/scorpio-id/kerberos/internal/iana/patype"
...
func NewASReqForTGT(realm string, c *krb5conf.Krb5Config, cname types.PrincipalName) (ASReq, error) {
...
func NewASReqForChgPasswd(realm string, c *krb5conf.Krb5Config, cname types.PrincipalName) (ASReq, error) {
	sname := types.PrincipalName{
...
func NewASReq(realm string, c *krb5conf.Krb5Config, cname, sname types.PrincipalName) (ASReq, error) {
...
func NewTGSReq(cname types.PrincipalName, kdcRealm string, c *krb5conf.Krb5Config, tgt Ticket, sessionKey types.EncryptionKey, sname types.PrincipalName, renewal bool) (TGSReq, error) {
...
func NewUser2UserTGSReq(cname types.PrincipalName, kdcRealm string, c *krb5conf.Krb5Config, clientTGT Ticket, sessionKey types.EncryptionKey, sname types.PrincipalName, renewal bool, verifyingTGT Ticket) (TGSReq, error) {
	a, err := tgsReq(cname, sname, kdcRealm, renewal, c)
	if err != nil {
		return a, err
	}
...
func tgsReq(cname, sname types.PrincipalName, kdcRealm string, renewal bool, c *krb5conf.Krb5Config) (TGSReq, error) {
	nonce, err := rand.Int(rand.Reader, big.NewInt(math.MaxInt32))
	if err != nil {
		return TGSReq{}, err
	}
