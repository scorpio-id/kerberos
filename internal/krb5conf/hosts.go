...
func (c *Krb5Config) GetKDCs(realm string, tcp bool) (int, map[int]string, error) {
	if realm == "" {
...
func (c *Krb5Config) GetKpasswdServers(realm string, tcp bool) (int, map[int]string, error) {
	kdcs := make(map[int]string)
	var count int
